...
<a class="nav-link" href="instructor-contact.jsp">Contact</a>
							...
							<li class="nav-item"><a class="nav-link"><%="Welcome, " + ind.getFname()%></a>
							</li>
						</ul>
					</div>
				</div>
					...
					<div class="" style="padding-bottom: 10px;">
						<h2 class="mb-3">
							...
							if (ind.getSubject().equals("CN")) {
								out.print("Upload Computer Network Videos");
							} else if (ind.getSubject().equals("ADA")) {
								out.print("Upload Analysis and Design of Algorithms Videos");
							} else if (ind.getSubject().equals("SE")) {
								out.print("Upload Software Engineering Videos");
							}
							%>
						</h2>
					</div>
						...
						<div class="desc">
							...
							<form action="InstructorController" method="post">
								<div class="form-group">
									<label for="exampleFormControlFile1">Example video Link
										input</label> <input type="url" name="Link" placeholder=""
										autocomplete="off" style="width: 60%">
									<!-- <input type="file" class="form-control-file" id="exampleFormControlFile1"> -->
								</div>
								<div class="form-group">
									<input type="hidden" name="id" class="form-control"
										value=<%=ind.getId()%> /> <input type="hidden" name="fname"
										class="form-control" value=<%=ind.getFname()%> /> <input
										type="hidden" name="lname" class="form-control"
										value=<%=ind.getLname()%> /> <input type="hidden"
										name="password" class="form-control"
										value=<%=ind.getPassword()%> /> <input type="hidden"
										name="contact" class="form-control"
										value=<%=ind.getContact()%> /> <input type="hidden"
										name="email" class="form-control" value=<%=ind.getEmail()%> />
									<input type="hidden" name="college" class="form-control"
										value=<%=ind.getCollege()%> /> <input type="hidden"
										name="subject" class="form-control"
										value=<%=ind.getSubject()%> /> <input type="hidden"
										name="uid" class="form-control" value=<%=ind.getUid()%> />
									<input type="hidden" name="year" class="form-control"
										value=<%=ind.getYear()%> />
								</div>
								<div class="form-group">
									<input type="submit" name="action" value="Upload"
										style="width: 40%" class="btn btn-primary">
								</div>
							</form>
						</div>
					</div>
